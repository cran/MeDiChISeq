...
function (data, chroms = NA, window = 6000, step.by = 5500, centers = NULL, 
    quiet = F, plot = F, nr.boots = 1, fit.res = 10, quant.cutoff = "q0.85", 
...
#         
        out.fits <- mclapply(posns, function(pos) {
            tmp.fit <- chip.deconv.seq(data = data, window = window, 
                ...
                verbose = verbose, fit.res = fit.res, nr.boots = nr.boots, 
                ...)
    ...
    get.fit.score <- function(par, pks, kernel = NULL, plot = F, 
        ...
        new.fits <- mclapply(1:nrow(pks), function(i) {
            ...
            if (is.null(datas[[i]])) 
                return(NULL)
            dat <- datas[[i]]
            chip.deconv.seq(data = dat, fit.res = 30, window = NA, 
...
function (x, ...) 
{
    ...
    cat("MeDiChI deconvolution over window:", obj[[1]]$window, 
        "\n")
    cat("Input arguments:\n")
    args <- c("center", "where", "fit.res", "max.steps", "nr.boots", 
