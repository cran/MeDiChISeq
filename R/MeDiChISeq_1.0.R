...
# Medichi main functions - adjusted for SEQ
                             ...
                             verbose = F, trace = F, plot.status = F, potential.good.locks=NULL) 
{
  ...
  while (iter.boot <= nr.boots) {
    ...
    if(sum(coeffs > coeff.cutoff.LARS)==0){
      ...
      if (require(quadprog, quietly = T, warn.conflicts = F) && best.step > 1 && n.coeffs[best.step] > 0 && sum(coeffs >  coeff.cutoff) > 0) {
        
        tmp.xx <- as.matrix(xx[, coeffs > coeff.cutoff, drop = F])
        Dmat <- t(tmp.xx) %*% tmp.xx
        if (require(corpcor, quietly = T, warn.conflicts = F) && !is.positive.definite(Dmat)) 
